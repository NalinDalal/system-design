# SEARCH AUTOCOMPLETE SYSTEM/ Design Top K/ Design Top K Most Searched Query
...
- fast response time(within 100 milliseconds as per facebook)
...
2. Traverse the subtree from the prefix node to get all valid children. A child is valid if it can form a valid query string. Time complexity: **O(c)** {bottleneck}
...
but the algo is slow, put optimisations:
...
3. store a prefix along with all of its completions together in the same node. Eliminates the O(n) bottleneck, but space increases

new complexity:

1. Find the prefix node. Time complexity: **O(1)**
2. Return top k. Since top k queries are cached, time complexity: **O(1)**.
...
since we need only O(1) reads, we don't care for storage
hence instead of storing in a conventional trie, we realized we can store our data in a hybrid data structure called a Prefix Hash Tree.
use a prefix hash tree: lookup operation lookup(key) can be used to support the canonical put(key,value) and get(key) hash table operations
...
while (lo ≤ hi) do
    mid ← (lo + hi)/2;
    /*Pmid(K) denotes prefix of K of length mid */
    node ← DHT-LOOKUP(Pmid(K));
    if (node is a leaf node) return node ;
    else
...
binary search reduces from O(D) to O(log D)

use a Range Query: 2 keys given L,H(L<=H), find K such that L<=k<=H
requires log D DHT lookups to locate leaf(L)

Insert: proportional to D{D is depth of leaf node K}
Delete: propotional to D

map the prefix to hashkey

alt to use redis(key: value)
store a user in redis with the `tenantKey`
prepend every key of a developer’s data with her tenant ID. So on the backend, all of our Redis keys look like this: `<tenantId>:<prefix>`.
then you can query this key-value pair
...
flowchart LR
  ...
  prefixy["Prefixy"]:::prefixy
  mongo[("MongoDB")]:::mongo
  redis[("Redis")]:::redis
  cli["CLI"]:::cli
  token["Token<br/>Generator"]:::token

  %% ===== Edges =====
  client -->|request| prefixy
  prefixy -->|response| client
  ...
  %% ===== Styles =====
  classDef client fill:#f7f3ee,stroke:#9a9a9a,stroke-width:2px,color:#6a6a6a,rx:10,ry:10;
  classDef prefixy fill:#cfe4f7,stroke:#9a9a9a,stroke-width:2px,color:#6a6a6a,rx:14,ry:14;
  classDef mongo fill:#79b35b,stroke:#4d7f3a,stroke-width:2px,color:#ffffff;
  classDef redis fill:#c8322a,stroke:#8e1f1a,stroke-width:2px,color:#ffffff;
  classDef cli fill:#3b3b3b,stroke:#2a2a2a,stroke-width:2px,color:#ffffff,rx:10,ry:10;
  classDef token fill:#9a9a9a,stroke:#6e6e6e,stroke-width:2px,color:#ffffff,rx:14,ry:14;
...
# [Implementation](https://github.com/NalinDalal/search-autocomplete-bun)
