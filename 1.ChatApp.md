...
- If a recipient is not online, hold the messages for that recipient on the server until they are online.
...
[Refer here](./3.NotificationSystem.md)
...
```mermaid
graph TD
    A["User A <br/> Phone - cur_max_msg_id: 653"] -->|WebSocket| C["Chat server 1 - sessions: phone, laptop"]
    B["User A <br/> Laptop - cur_max_msg_id: 842"] -->|WebSocket| C
    C --> D["KV store"]
